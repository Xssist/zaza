...
export default async function run(page, ui) {
  ...
  results.crumbsJsonLd = await page.evaluate(() => {
    ...
    try {
      ...
      return d["@type"] === "BreadcrumbList" && d.itemListElement.length === 4;
    } catch {
      return false;
    }
  });
  results.favicon = await page.evaluate(
  ...
  results.config = await page.evaluate(() => {
    ...
    return {
      ...
      bioNonEmpty:
        typeof c?.profile?.bio === "string" && c.profile.bio.length > 0,
      tracks: c?.music?.tracks?.length,
      noMojibake:
        !/Ã|ð/.test(JSON.stringify(c)) &&
        !/Ã|ð/.test(document.documentElement.innerHTML),
      seoDescriptionOk:
        typeof c?.seo?.description === "string" &&
        !/part time/.test(c.seo.description),
    };
  });
  ...
  results.loadingHidden = await page.evaluate(
    () =>
      document.querySelector("#loading-screen")?.classList.contains("hidden") ??
      "missing",
  );
  ...
  results.username = await page.evaluate(
    () => document.querySelector("#profile-username")?.textContent,
  );
  results.bioRendered = await page.evaluate(() =>
    (document.querySelector("#profile-bio")?.textContent || "").slice(0, 40),
  );
  results.socialLinks = await page.evaluate(() =>
    [...document.querySelectorAll("#socials-container a")].map((a) => a.href),
  );
  results.avatarImg = await page.evaluate(
    () =>
      !!document.querySelector(
        "#avatar-wrap img, #avatar-wrap .avatar-initial",
      ),
  );

  // 5. Simulate "entered" state and exercise keyboard shortcuts
  await page.evaluate(() => {
    window.dispatchEvent(new Event("audit-enter"));
  });
  await page.evaluate(() => {
  ...
  results.ctxMenuOpen = await page.evaluate(
    () =>
      document.querySelector("#ctx-menu")?.classList.contains("open") ?? false,
  );
  results.ctxItemCount = await page.evaluate(
    () => document.querySelectorAll("#ctx-menu .ctx-item").length,
  );
  await page.keyboard.press("Escape");
  await page.waitForTimeout(200);
  results.ctxMenuClosed = await page.evaluate(
    () => !document.querySelector("#ctx-menu")?.classList.contains("open"),
  );
  ...
  await page.waitForTimeout(300);
  const termBuilt = await page.evaluate(
    () => !!document.getElementById("egg-terminal"),
  );
  if (termBuilt) {
    ...
    results.eggTerminal = await page.evaluate(() => ({
      built: true,
      output: document
        .getElementById("egg-output")
        ?.textContent?.includes("command not found: frobnicate"),
      noErrorEcho: errors.every((e) => !/frobnicate|egg/.test(e)),
    }));
  } else {
    results.eggTerminal = {
      built: false,
      note: "keystroke detector did not trigger (synthetic events lack timing density)",
    };
  }
